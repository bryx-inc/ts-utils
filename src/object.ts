...
export function dropNullValues<T extends object>(obj: T) {
  ...
  return Object.keys(obj).reduce((acc, cur: string) => {
    if (
      typeof obj[cur as keyof T] != "undefined" &&
      obj[cur as keyof T] !== null
    )
      acc[cur] = obj[cur as keyof T] as PermittedValue;
    return acc;
 ...
 */
export function derecordify<
  T extends object,
  KN extends string,
  VN extends string
>(record: T, opts: { k: KN; v: VN }) {
 ...
 * // }
 */
export function recordify<
  T extends object,
  K extends keyof { [Key in keyof T]-?: T[Key] extends string ? Key : never }
>(arr: T[], key: K) {
  return arr.reduce((obj, cur) => {
    if (typeof obj[cur[key] as string] == "undefined")
      obj[cur[key] as string] = [] as T[];
 ...
 */
export function getPropertyUnsafe<T extends {}, E, K extends string>(
  v: T,
  key: K
): Maybe<E> {
