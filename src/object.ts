...
export function dropNullValues<T extends object>(obj: T) {
    type PermittedValue = T extends { [K in keyof T]: Exclude<infer V, "undefined" | "null"> } ? V : never;
...
export function dropKeys<T extends object, K extends (keyof T)[]>(from: T, keys: K) {
    const o = Object.create(from); // clone object
...
export function selectKeys<T extends object, K extends (keyof T)[]>(from: T, keys: K) {
    const o = Object.create(from);
    for (const k of keys) if (!keys.includes(k)) delete o[k];

    return o as { [k in K[number]]: T[k] };
}
...
export function derecordify<T extends object, KN extends string, VN extends string>(record: T, opts: { k: KN; v: VN }) {
    return Object.entries(record).map(([k, v]) => ({ [opts.k]: k, [opts.v]: v })) as {
 ...
 */
export function getPropertyUnsafe<T extends {}, E, K extends string>(v: T, key: K): Maybe<E> {
